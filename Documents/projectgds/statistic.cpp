...
Statistic::Statistic(QVector<double>* ticks,QVector<QString> *labels)
{
    QSqlQuery qry;
    int i=0;
    qry.exec("SELECT SEXE FROM EMPLOYEE");
    while (qry.next())
    {
        QString Sexe = qry.value(0).toString();
        i++;
        *ticks<<i;
        *labels <<Sexe;
    }
...
void Statistic::MakeStat()
{
    QLinearGradient gradient(0, 0, 0, 400);
                  ...
                  amande->setStackingGap(1);
                   //couleurs
                  amande->setName("Repartition de sexe ");
                  ...
                  QSqlQuery q1("SELECT count(SEXE) FROM EMPLOYEE WHERE SEXE='mas'");
                  while (q1.next())
                  {
